...
<html lang="en">
    ...
    <body>
        ...
        if(isset($_GET['dcity'])){
            ...
            $sql = "SELECT * FROM trains WHERE (Station_Code = '$ocode' OR Station_Code = (SELECT alt2 FROM alternate WHERE alt1 = '$ocode') OR Station_Code = (SELECT alt3 FROM alternate WHERE alt1 = '$ocode') OR Station_Code = (SELECT alt4 FROM alternate WHERE alt1 = '$ocode')) AND (Train_No IN (SELECT Train_No FROM trains WHERE (Station_Code = '$dcode'  OR Station_Code = (SELECT alt2 FROM alternate WHERE alt1 = '$dcode') OR Station_Code = (SELECT alt3 FROM alternate WHERE alt1 = '$dcode') OR Station_Code = (SELECT alt4 FROM alternate WHERE alt1 = '$dcode'))))";
            $result = mysqli_query($conn,$sql);
            
            while($row = mysqli_fetch_assoc($result)){
                 echo "Train_No: ".$row['Train_No'].". Distance is ".$row['Distance']."km. Train Name is: ".$row['Train_Name']."<br>";
            }
